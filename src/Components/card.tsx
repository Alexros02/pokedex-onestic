...
/**
 * Componente de tarjeta estilo "glass" para mostrar un Pokémon.
 * - Renderiza información resumida de un Pokémon y permite navegar al detalle.
 * - Accesible mediante teclado (Enter/Espacio) y con `aria-label` descriptiva.
 * - Usa colores de acento derivados del tipo del Pokémon.
 * - Incluye botón de favoritos fijado en la esquina inferior derecha.
 */
const Card = ({ pokemon }: CardProps) => {
  ...
  return (
    <div className="flex justify-center">
      <div
        className="relative w-72 h-[25rem] sm:w-80 rounded-2xl border border-white/25 bg-white/30 dark:bg-white/5 backdrop-blur-xl shadow-xl overflow-hidden cursor-pointer group transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl hover:bg-white/40 dark:hover:bg-white/10"
        role="button"
      ...
      >
        ...
        <div className="relative z-10 px-4 pt-4 pb-2 flex items-center justify-between">
          <h3 className="text-xl font-semibold tracking-tight">{capitalizeFirst(pokemon.name)}</h3>
          <span
